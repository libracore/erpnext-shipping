...
class LetMeShipUtils():
    ...
    def get_available_services(self, delivery_to_type, pickup_address,
        ...
        try:
            ...
            if response.status_code != 200:
                frappe.throw(_('An Error occurred while fetching LetMeShip prices: {0}<br>Data:<br>{1}')
                    .format(response.text, payload))
            response_data = response.json()
            if 'serviceList' in response_data:
                for r in response_data['serviceList']:
                    available_service = self.get_service_dict(r)
    ...
    def set_letmeship_specific_fields(self, pickup_contact, delivery_contact):
        if pickup_contact.phone:
            ...
            frappe.throw( _("Please configure first name, last name and phone in the user profile") )
